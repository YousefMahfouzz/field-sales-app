...
export default function PublicHomePage() {
  ...
  useEffect(() => {
    supabase
      ...
      .select(`
        id, display_order,
        product:products(id, name, description, image_url, images, brand, category)
      `)
      .eq('is_active', true)
      .order('display_order')
      .then(({ data }) => {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0a0a0a', color:'white', fontFamily:'-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', maxWidth:'100%' }}>
      ...
      <div style={{ paddingTop:120, paddingBottom:80, textAlign:'center', padding:'120px 24px 80px' }}>
        ...
        <h1 style={{ fontSize:'clamp(36px, 6vw, 72px)', fontWeight:900, lineHeight:1.1, marginBottom:20, letterSpacing:'-2px', background:'linear-gradient(135deg, #fff 0%, rgba(255,255,255,0.6) 100%)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent' }}>
          Premium Products.<br/>Built for Distributors.
        </h1>
        <p style={{ fontSize:'clamp(16px, 2vw, 20px)', color:'rgba(255,255,255,0.5)', maxWidth:480, margin:'0 auto 40px', lineHeight:1.7 }}>
          Quality wholesale products for beauty supply stores, gas stations, convenience stores and more.
        </p>
        <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
          <button onClick={() => navigate('/login')} style={{
            padding:'14px 32px', borderRadius:12, border:'none', cursor:'pointer',
            background:'linear-gradient(135deg,#6366f1,#2563eb)', color:'white',
            fontWeight:700, fontSize:16, boxShadow:'0 8px 24px rgba(99,102,241,0.4)',
          }}>
            Distributor Login →
          </button>
          <button onClick={() => document.getElementById('products').scrollIntoView({ behavior:'smooth' })} style={{
            padding:'14px 32px', borderRadius:12, border:'1px solid rgba(255,255,255,0.15)', cursor:'pointer',
            background:'rgba(255,255,255,0.05)', color:'white', fontWeight:600, fontSize:16,
          }}>
            View Products ↓
          </button>
        </div>
      ...
      <div id="products" style={{ maxWidth:1200, margin:'0 auto', padding:'0 24px 100px' }}>
        <div style={{ textAlign:'center', marginBottom:56 }}>
          ...
          <p style={{ color:'rgba(255,255,255,0.4)', fontSize:16 }}>Curated wholesale selection for retail distributors</p>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(300px, 1fr))', gap:28 }}>
          {featured.map((product, i) => (
            <div
              ...
              onMouseLeave={e => { e.currentTarget.style.transform='translateY(0)'; e.currentTarget.style.border='1px solid rgba(255,255,255,0.08)'; e.currentTarget.style.background='rgba(255,255,255,0.04)' }}
            >
              {/* Image */}
              ...
              <div style={{ padding:'20px 22px 24px' }}>
                {product.brand && (
                  <p style={{ fontSize:11, color:'rgba(99,102,241,0.8)', fontWeight:700, letterSpacing:'1px', textTransform:'uppercase', marginBottom:6 }}>{product.brand}</p>
                )}
      ...
      {/* CTA */}
      <div style={{ background:'linear-gradient(135deg, rgba(99,102,241,0.15), rgba(37,99,235,0.1))', borderTop:'1px solid rgba(255,255,255,0.06)', padding:'80px 24px', textAlign:'center' }}>
        <h2 style={{ fontSize:'clamp(24px, 4vw, 40px)', fontWeight:800, marginBottom:16 }}>Ready to start distributing?</h2>
        <p style={{ color:'rgba(255,255,255,0.4)', fontSize:16, marginBottom:32, maxWidth:400, margin:'0 auto 32px' }}>
          Log in to access your personalized price list and place orders.
        </p>
        <button onClick={() => navigate('/login')} style={{
          padding:'16px 40px', borderRadius:12, border:'none', cursor:'pointer',
          background:'linear-gradient(135deg,#6366f1,#2563eb)', color:'white',
          fontWeight:700, fontSize:17, boxShadow:'0 8px 30px rgba(99,102,241,0.4)',
        }}>
          ...
          <div onClick={e => e.stopPropagation()} style={{ background:'#111', borderRadius:24, maxWidth:560, width:'100%', overflow:'hidden', border:'1px solid rgba(255,255,255,0.1)' }}>
            ...
            <div style={{ padding:28 }}>
              ...
              <div style={{ background:'rgba(255,255,255,0.05)', borderRadius:12, padding:'14px 18px', marginBottom:20, border:'1px solid rgba(255,255,255,0.08)' }}>
                <p style={{ color:'rgba(255,255,255,0.4)', fontSize:13 }}>💡 Log in to see pricing and place orders for this product.</p>
              </div>
              <div style={{ display:'flex', gap:10 }}>
                <button onClick={() => navigate('/login')} style={{ flex:1, padding:'13px', borderRadius:10, border:'none', cursor:'pointer', background:'linear-gradient(135deg,#6366f1,#2563eb)', color:'white', fontWeight:700, fontSize:15 }}>Login for Pricing →</button>
                <button onClick={() => setSelected(null)} style={{ padding:'13px 18px', borderRadius:10, border:'1px solid rgba(255,255,255,0.15)', cursor:'pointer', background:'transparent', color:'white', fontWeight:600, fontSize:15 }}>✕</button>
              </div>
            </div>
          </div>
        </div>
